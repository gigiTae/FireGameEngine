...
void ImpToolModule::ResourceViewer::Update()
{
	...
	if (ImGui::Begin("ResourceViewer"))
	{
		...
		_buttonCount = 0;
		_maxButtonCount = static_cast<unsigned int>(_viewerWidth / _fileButtonSize.x);
...
void ImpToolModule::ResourceViewer::ViewEditFile()
{
	...
	if (_isOpenDeletePopup)
	{
		...
		if (ImGui::BeginPopupModal("Enter Directory Name", NULL, ImGuiWindowFlags_AlwaysAutoResize))
		{
...
void ImpToolModule::ResourceViewer::ViewCurrentPath()
{
...
void ImpToolModule::ResourceViewer::ViewDirectory(const std::filesystem::path& path)
{
	...
	if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_SourceAllowNullID))
	{
		...
		ImGui::SetDragDropPayload("ResourcePath", entPath.c_str(), (entPath.size() + 1) * sizeof(wchar_t));
		...
		if (payLoad)
		{
			...
			if (std::filesystem::exists(status) && std::filesystem::is_regular_file(status))
			{
			...
			else if (std::filesystem::is_directory(path)) // 파일폴더인경우
 			{
				...
				if (iter == std::filesystem::end(iter)) // 파일내부에 아무것도 없는경우
				{
					std::filesystem::remove(path);
				}
			}
