...
FBXLoad::MeshLoader::MeshLoader(FbxManager* manager)
	: _manager(manager)
{
	assert(_manager);
}
...
void FBXLoad::MeshLoader::LoadMesh(FbxNode* node)
{
	...
	if (mesh->RemoveBadPolygons() < 0) return;
	...
	// 가능한 모든 점을 삼각형 폴리곤으로 바꿈
	{
		...
		if (!mesh || mesh->RemoveBadPolygons() < 0) return;
	}
	...
	for (i = 0; i < polygonCount; i++)
	{
		...
		for (j = 0; j < polygonSize; j++)
		{
			...
			else
			{
				...
				// 위치값을 담는다.
				_meshes.back()->_meshVertex[i * 3 + j]->_localPos = {
			...
			// 컬러값 데이터 유무 표시한다.
			if (eColorCount) _meshes.back()->_hasNormal = true;
			...
			for (l = 0; l < eColorCount; l++)
			{
				...
				case FbxGeometryElement::eByControlPoint:
					...
					case FbxGeometryElement::eDirect:
					{
						...
						FBXSDK_printf("%s %f, %f, %f, %f\n", header, vertexColor[0], vertexColor[1], vertexColor[2], vertexColor[3]);
					}
					break;
			...
			for (l = 0; l < mesh->GetElementUVCount(); ++l)
			{
				...
				case FbxGeometryElement::eByControlPoint:
					...
					case FbxGeometryElement::eIndexToDirect:
					{
						...
						FBXSDK_printf("%s %f, %f\n", header, leUV->GetDirectArray().GetAt(id)[0], leUV->GetDirectArray().GetAt(id)[1]);
					}
					break;
					default:
						break; // other reference modes not shown here!
				...
				// uv 좌표값을 담는다.
				_meshes.back()->_meshVertex[i * 3 + j]->_uvs.push_back(Vector2{ static_cast<float>(uv[0]), static_cast<float>(uv[1]) });
			...
			// 노말 값의 유무 표시한다.
			if (normalCount) _meshes.back()->_hasColor = true;
			...
			// 탄젠트 값의 유무를 표시한다. 
			if (tangentCount) _meshes.back()->_hasColor = true;
			...
			for (l = 0; l < mesh->GetElementBinormalCount(); ++l)
			{
				...
				// 바이노말 값을 담는다.
				_meshes.back()->_meshVertex[i * 3 + j]->_bitangent = {
